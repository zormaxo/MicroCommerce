...
public class BasketService : IBasketService
{
    ...
    {
        var model = new { CatalogItemId = productId, Quantity = 1, BasketId = identityService.GetUserName() };
        try
        {
            await apiClient.PostAsync("basketAgg/items", model);
        }
    ...
    public Task Checkout(BasketDTO basket) { return apiClient.PostAsync("basket/checkout", basket); }
